      ...
      await axios
        ...
        .then((response) => {
          if (response.data.event) {
            ...
            const day = eventDate[0]
            const month = eventDate[1] - 1;
            const year = eventDate[2].split(',')[0]; 

            form.setValue("title", event.title);
            form.setValue("description", event.description);
            form.setValue("category", event.category);
            form.setValue("meetlink", event.meetlink);
            form.setValue("date", new Date(year, month, day));
            form.setValue("time", event.time);
          }
        })
        .catch((error) => {
          console.error(error);
          toast({ description: "Failed to fetch event. Please try again later." });
        });
    }
    fetchEvent();
  }, [eventId]);
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormDescription>Warning: Please re-uplaod the event cover</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
